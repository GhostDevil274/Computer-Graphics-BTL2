...
void main() {
    ...
    if (render_mode == 0) baseCol = flat_color;
    else if (render_mode == 1 || render_mode == 2) baseCol = vertexColor;
    else baseCol = texture(tex_diffuse, TexCoord).rgb;

    if (render_mode == 0 || render_mode == 1 || render_mode == 3) {
        FragColor = vec4(baseCol, 1.0);
    } else {
        vec3 lighting = 0.2 * baseCol;
